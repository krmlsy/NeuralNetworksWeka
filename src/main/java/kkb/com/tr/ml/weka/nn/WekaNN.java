...
public class WekaNN {
	...
	public static void main(String[] args) {
		...
		"-L "+ 0.1//learning rate
                +" -M "+0		//momentum
                +" -N "+100000 	//epoch. the larger epoch, the better results.
                +" -V "+0 		//validation
                +" -S "+0 		//seed
                +" -E "+ 0		//error
                +" -H "+"3,3"; 	//hidden nodes. e.g. use "3,3" for 2 level hidden layer with 3 nodes
		
		String fileType = "arff"; //available file types: arff, txt
		String dumpLocation = System.getProperty("user.dir")+"\\trained\\"; //specify where you want to store the binary network
		...
		try{
			...
			*/
			...
			System.out.println("PCTCorrect: "+eval.errorRate());
	...
	public static Instances retrieveHistoricalData(String fileType){
		...
		try{
			...
			if("arff".equals(fileType)){
				...
				String historicalDataPath = System.getProperty("user.dir")+"\\dataset\\New.arff";
